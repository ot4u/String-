...
START_TEST(test_memcpy_different_sizes)  // проверяет копирование данных
                                         // различной длины
{
...
START_TEST(test_memset_basic)  // проверяет базовое использование функции
                               // memset, где все байты в строке устанавливаются
...
START_TEST(test_memset_null_pointer)  // проверяет поведение функции при
                                      // передаче нулевого указателя
{
  char *str = "";
  size_t n = 0;
...
START_TEST(test_memset_zero_length)  // проверяет поведение функции при
                                     // установке длины в ноль
{
  char str[] = "Hello, World!";
  size_t n = 0;
...
START_TEST(test_memset_single_byte)  // проверяет, что функция работает
                                     // корректно для строки из одного символа
{
  char str[] = "A";
  size_t n = 1;
...
START_TEST(
    test_memset_large_characters_partial)  // устанавливает первую половину
                                           ...
                                           // возможное значение и проверяет,
...
START_TEST(test_strncat_zero_length)  // проверяет поведение функции при
                                      // передаче длины равной нулю
{
  char dest[20] = "Hello";
  const char *src = " World!";
  size_t n = 0;
...
START_TEST(
    test_strncat_src_length_shorter_than_n)  // проверяет склеивание строк,
                                             // когда длина источника меньше
                                             // значения n
{
  char dest[20] = "Hello";
  const char *src = " W";
...
START_TEST(test_strchr_basic)  // проверяет базовое использование функции
                               // strchr, когда символ присутствует в строке
{
  const char *str = "Hello, World!";
  const int c = 'o';
...
START_TEST(test_strncpy_empty_source)  // проверка, что функция корректно
                                       // обрабатывает пустую строку и копирует
                                       // ее в буфер назначения
{
  const char *src = "";
  char dest[20];
  size_t n = 5;
...
START_TEST(
    test_strncpy_null_destination_and_source)  // проверка, что функция
                                               // корректно обрабатывает случай,
                                               // когда указатели на буфер
                                               // назначения и исходную строку
                                               // равны NULL
{
  const char *src = "";
  char *dest = "";
...
START_TEST(
    test_strerror_invalid_error_code_high)  // проверяет, что функция
                                            //  s21_strerror возвращает
                                            //  ненулевой
                                            // результат для невалидного
                                            // высокого значения кода ошибки
{
  int errnum = 1000;
  const char *result_my = s21_strerror(errnum);
...
START_TEST(test_strlen_non_empty_string)  // проверяет, что длина непустой
                                          // строки корректно вычисляется
{
  const char *str = "Hello, World!";
  size_t result_my = s21_strlen(str);
  size_t result_std = strlen(str);
...
START_TEST(
    test_strlen_single_character)  // проверяет, что функция правильно
                                   // определяет длину строки с одним символом
{
  const char *str = "A";
  size_t result_my = s21_strlen(str);
  size_t result_std = strlen(str);
...
START_TEST(
    test_strlen_utf8_string)  // проверяет, что функция правильно
...
START_TEST(test_strpbrk_no_match)  // проверяет, что функция возвращает NULL,
                                   // если в строке str1 нет ни одного
                                   // совпадения с символами из str2
{
  const char *str1 = "abcdef";
  const char *str2 = "xyz";
  const char *result_my = s21_strpbrk(str1, str2);
...
START_TEST(
    test_strrchr_multiple_occurrences)  // проверяет, что функция правильно
                                        ...
                                        // в строке str с несколькими
                                        // вхождениями этого символа
{
...
START_TEST(
    test_strstr_match)  // проверяет, что функция правильно находит первое
...
START_TEST(test_strstr_match_beginning)  // проверяет, что функция правильно
                                         // находит подстроку needle,
                                         // находящуюся в начале строки haystack
{
  const char *haystack = "Hello, world!";
...
START_TEST(
    test_strstr_match_end)  // проверяет, что функция правильно находит
                            // подстроку
...
START_TEST(
    test_strtok_delimiters_only)  // проверяет, что функция возвращает NULL,
                                  // если строка содержит только разделители.
{
  char str[30] = ",,,";
  char str2[30] = ",,,";
  ck_assert_msg(s21_strtok(str, " .,") == strtok(str2, " .,"), "FAILURE!");
...
START_TEST(
    test_trim_multiple_chars)  // Обрезка нескольких символов, указанных в
                               //  trim_chars
{
  const char *src = "!!Hello, World!!!";
  const char *trim_chars = "! ";
